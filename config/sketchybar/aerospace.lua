...
local function workspace_changed(env)
    local id = env.FOCUSED_WORKSPACE
    if spaces[tonumber(id)] ~= nil and spaces[tonumber(id)].name == env.NAME then
...
local function highlight_active_space()
...
local function recreate_spaces()
...
local function update_windows(env)
    sbar.exec(
