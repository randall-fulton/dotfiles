...
To trigger, mark a selection, then use =C->= to add the next match, =C-<= to add the previous, or =C-c C-<= to add all matches.

While using multiple cursors =C-j= must be used for newlines.
#+begin_src elisp
  (use-package multiple-cursors
    :bind (("C-c >" . 'mc/mark-next-like-this)
	   ("C-c <" . 'mc/unmark-previous-like-this)))
#+end_src
...
Highlight =TODO= comments. This needs to be manually added as a hook on target major mode. It is not enabled globally.
#+begin_src elisp
  (defun rf/buffer-highlight-todo (&optional buf)
    "Enable highlighting of TODO comments in BUF. Will target active
     buffer when BUF is not provided."
    (interactive)
